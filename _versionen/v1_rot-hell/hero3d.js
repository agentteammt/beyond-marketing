...
(function () {
  ...
  function init(mount, cfg) {
    ...
    try {
      ...
      let station = null, ringInner = null, winGlow = null, win = null, debris = null, imagePlane = null;
      const navLights = [];
      ...
      function tick() {
        ...
        if (imagePlane) {
          // floating photo panel: tilt toward pointer (+ drag), gentle bob
          imagePlane.rotation.y += (((PRM ? 0 : pointerX * 0.10) + curRotY) - imagePlane.rotation.y) * 0.06;
          imagePlane.rotation.x += (((PRM ? 0 : -pointerY * 0.07) + curRotX) - imagePlane.rotation.x) * 0.06;
          imagePlane.position.x += ((PRM ? 0 : -pointerX * 0.45) - imagePlane.position.x) * 0.05;
          imagePlane.position.y += (((PRM ? 0 : pointerY * 0.32) + (PRM ? 0 : Math.sin(t * 0.5) * 0.12)) - imagePlane.position.y) * 0.05;
        }

        // camera parallax + scroll dolly
        if (!PRM) {
          const camPX = imagePlane ? pointerX * 0.3 : pointerX * 0.9;
          const camPY = imagePlane ? -pointerY * 0.2 : 0.6 - pointerY * 0.6;
