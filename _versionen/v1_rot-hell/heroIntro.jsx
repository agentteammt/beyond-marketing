...
function Typewriter({ lead, accent, speed = 34, play, done, onDone }) {
  ...
  return (
    <h1 aria-label={lead + " " + accent} style={{ fontSize: "clamp(40px,7vw,88px)", marginBottom: 8, minHeight: "1.96em", lineHeight: "0.85" }}>
      <span style={{ fontSize: "clamp(40px,14vw,75px)" }}>{line1}{caretOnLine1 ? <Caret /> : null}</span><br />
      <span style={{ color: "var(--accent)", textShadow: "0 0 calc(50px*var(--glow)) color-mix(in srgb,var(--accent) 55%, transparent)", fontSize: "clamp(40px,14vw,75px)", lineHeight: "0.88" }}>{line2}{caretOnLine2 ? <Caret /> : null}</span>
    </h1>);
