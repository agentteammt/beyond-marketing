...
const HERO_INTRO_TIMING = {
  desktop: {
    ...
    typeSpeedMs: 34,
    ctaFadeMs: 440,
    parallax: 1, // Stärke der Tiefen-Parallaxe beim Anflug (0..1)
...
function heroEaseOutCubic(t) {t = Math.min(1, Math.max(0, t));return 1 - Math.pow(1 - t, 3);}
...
function useHeroIntro() {
  ...
  const skip = R.useCallback(() => {setPhase("done");}, []);
  ...
  R.useEffect(() => {
    ...
    return () => {clearTimeout(armed);window.removeEventListener("keydown", onSkip);window.removeEventListener("pointerdown", onSkip);};
    // eslint-disable-next-line
  }, [active, phase]);

  return { active, phase, skip, mobile, reduce, timing };
}

/* Typewriter — tippt `lead` + (farbige) `accent`-Zeile zeichenweise ein.
   play: tippen aktiv · done: sofort vollständig (kein Caret). */
function Typewriter({ lead, accent, speed = 34, play, done, onDone }) {
  const R = React;
  ...
  R.useEffect(() => {
    ...
    let i = 0;setN(0);doneRef.current = false;
    const id = setInterval(() => {
      i++;setN(i);
      if (i >= full.length) {clearInterval(id);if (!doneRef.current) {doneRef.current = true;onDone && onDone();}}
    }, speed);
  ...
  const Caret = () => <span className="tw-caret" aria-hidden="true">▍</span>;
  return (
    <h1 aria-label={lead + (accent ? " " + accent : "")} style={{ fontSize: "clamp(40px,7vw,88px)", marginBottom: 8, minHeight: accent ? "1.96em" : "0.95em", lineHeight: "0.85" }}>
      <span style={{ fontSize: "clamp(40px,14vw,75px)" }}>{line1}{caretOnLine1 ? <Caret /> : null}</span>
      {accent ? <React.Fragment><br /><span style={{ color: "var(--accent)", textShadow: "0 0 calc(50px*var(--glow)) color-mix(in srgb,var(--accent) 55%, transparent)", fontSize: "clamp(40px,14vw,75px)", lineHeight: "0.88" }}>{line2}{caretOnLine2 ? <Caret /> : null}</span></React.Fragment> : null}
    </h1>);

}

Object.assign(window, { HERO_INTRO_TIMING, heroEaseOutCubic, useHeroIntro, Typewriter });